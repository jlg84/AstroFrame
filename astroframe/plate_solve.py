...
class SolveCache:
    ...
    def load_record(self, image_path: str | Path) -> dict[str, Any] | None:
    ...
    def save(
        ...
        *,
        ...
        cache_path = self._path_for(image_path)
        previous = self.load_record(image_path) or {}
        payload = {
            "schema_version": 2,
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="astroframe_astap_") as temp_dir:
            ...
            if estimated_width_deg and estimated_width_deg > 0:
                # ASTAP expects the approximate image height, not width.
                estimated_height = (
    ...
    def wait_for_job(
        ...
        *,
        ...
        while time.monotonic() - started < timeout_seconds:
            ...
            if progress:
                progress(
                    f"Astrometry.net accepted the image • queued "
                    f"{_elapsed_label(started)} • submission #{submission_id}"
                )
            waited = 0.0
    ...
    def wait_for_solution(
        ...
        *,
        ...
        while time.monotonic() - started < timeout_seconds:
            ...
            if progress:
                progress(
                    f"Astrometry.net is solving • {_elapsed_label(started)} "
                    f"• job #{job_id}"
                )
            waited = 0.0
            while waited < poll_seconds:
